...
- **tools/**: `install_sdl2_toolchain.sh`(MSYS2 환경 구축), 빌드/런 bat, `verify_fixwin3.ps1`(DPI-aware 화면 검증 — `ARCHITECTURE_DOCS/15_verification_playbook.md`), `app_mouse_test3.ps1`(드래그 테스트 드라이버), `probe_mouse_scaling.ps1`(마우스/DPI 배율 실측 — 15 문서 §9), `fix_bom.ps1`(ps1 BOM 복구)
