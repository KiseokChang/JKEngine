...
// OccMapPanel
// ---------------------------------------------------------------------------
OccMapPanel::OccMapPanel(const JKRect& rect, const std::vector<OccTarget>& targets,
...
void OccMapPanel::OnPaintClient(JKDC& dc) {
...
void OccMapPanel::RespondMessage(const JKEvent& ev) {
...
JKPoint OccMapPanel::WorldToScreen(const JKRect& rc, const OccTarget& t) {
    return JKPoint{ rc.x + t.x * rc.w / 2000, rc.y + t.y * rc.h / 2000 };
}

size_t OccMapPanel::TypeIndexOf(const std::string& type) {
...
void OccMapPanel::DrawGrid(JKDC& dc, const JKRect& rc) const {
...
void OccMapPanel::DrawTypeGlyph(JKDC& dc, int px, int py, size_t ti) const {
    if (ti == 3) { // Air: 다이아몬드
...
void OccMapPanel::DrawTarget(JKDC& dc, const JKRect& rc, size_t i) const {
...
JKPoint OccMapPanel::WorldToScreenU(const JKRect& rc, const OccUnit& u) {
    return JKPoint{ rc.x + u.x * rc.w / 2000, rc.y + u.y * rc.h / 2000 };
}

void OccMapPanel::DrawUnitGlyph(JKDC& dc, int px, int py) const {
    // Friendly unit: filled blue 12-gon (integer circle approximation).
    static const int32_t kDx[12] = { 8, 7, 4, 0, -4, -7, -8, -7, -4, 0, 4, 7 };
    static const int32_t kDy[12] = { 0, 4, 7, 8, 7, 4, 0, -7, -8, -7, -4, 0 };
...
void OccMapPanel::DrawUnit(JKDC& dc, const JKRect& rc, size_t i) const {
...
void OccMapPanel::DrawLegend(JKDC& dc, const JKRect& rc) const {
...
// OccUI
// ---------------------------------------------------------------------------
OccUI::OccUI(std::function<void()> onExit) : onExit_(std::move(onExit)) {
}
...
void OccUI::BuildMainWindow() {
    ...
    menu->AddMenu("Help", {
        jk::JKMenuItem{ "About", 305, [this]() {
            if (aboutBox_ && g_jkAppHost && !g_jkAppHost->GetModalWindow()) {
                aboutBox_->Show();
            }
        }},
    });
...
std::unique_ptr<JKWindow> OccUI::TakeMainWindow() {
    return std::move(mainWindow_);
}

void OccUI::RefreshTargetList() {
    if (!targetList_) {
        return;
    }
    targetList_->Clear();
...
void OccUI::ShowTargetDialog(bool modify) {
    OccTarget base;
    int idx = -1;
    if (modify) {
        int sel = targetList_->GetSelectedIndex();
    ...
    targetDlg_->SetOnClose([this](int) {
        if (g_jkAppHost) {
            g_jkAppHost->SetModalWindow(mainWindow_.get());
        }
        RefreshTargetList();
    });
    targetDlg_->Show();
...
void OccUI::ShowUnitDialog(bool modify) {
    OccUnit base;
    int idx = -1;
    if (modify) {
        int sel = unitList_->GetSelectedIndex();
...
void OccUI::ShowMessageModal(const std::string& title, const std::string& msg,
