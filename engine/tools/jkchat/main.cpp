...
// The LLM engine lives in jkcore now (JKLlmEngine — extracted for jkbridge);
// this file keeps only the Win32 face of it: callbacks that PostMessage the
// same WM_APP_* events the worker used to post directly.
using LlmTurnResult = jk::agent::LlmTurnResult;
...
// --- LLM turn (claude headless — JKLlmEngine in jkcore) ---------------------
...
static std::string g_sessionId;  // claude session continuity (--resume) —
                                 // the consumer's state (JKLlmEngine contract)
static jk::agent::JKLlmEngine g_llm;

// JKLlmEngine callbacks fire on the WORKER thread — PostMessage is the
...
static void OnLlmDone(LlmTurnResult&& r, void*) {
    PostMessageW(g_hMain, WM_APP_LLM_DONE, 0,
                 reinterpret_cast<LPARAM>(new LlmTurnResult(std::move(r))));
}

static void StartLlmTurn(const std::string& prompt) {
    if (!g_llm.StartTurn(prompt, g_sessionId, OnLlmDelta, OnLlmDone, nullptr)) {
