...
function Invoke-ConquestCycle([string]$tag) {
    ...
    #     (list_windows rects are logical; the server converts to surface px).
    ...
    Check "$tag-drive-click" ($r -match 'sent\\":true')
    Start-Sleep -Milliseconds 700
    # (4) verify: capture hash changed (the click moved pixels) + the window
    #     is still alive in list_windows under the same id.
    $h1 = Capture-Hash $win.id
    Check "$tag-verify-hash" ($h1 -ne "" -and $h1 -ne $h0)
    $again = Get-MineWindow
    Check "$tag-verify-alive" ($null -ne $again -and $again.id -eq $win.id)
    if (-not (($h0 -ne "") -and ($r -match 'sent\\":true') -and
        ($h1 -ne "" -and $h1 -ne $h0) -and ($null -ne $again -and $again.id -eq $win.id))) {
        $script:cycleOk = $false
    }
}
