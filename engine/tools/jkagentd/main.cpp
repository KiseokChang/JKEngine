...
"{\"name\":\"launch_app\",\"description\":\"Launch an app. For built-in apps use app with a built-in name such as minesweeper, tetris, browser, taskbar, vplayer, terminal (a jkapp_<name>.dll must exist next to jkdesktop.exe; the server verifies and replies ok:false unknown_app otherwise). For .jkx packages use jkx with a path or a bare package name (resolved to apps/<name>.jkx) — workshop is a .jkx package, so launch it as {jkx:'workshop'} (the server also accepts {app:'workshop'} and resolves it to the package). Do NOT pass built-in app names via jkx\",\"inputSchema\":{\"type\":\"object\",\"properties\":{\"app\":{\"type\":\"string\"},\"jkx\":{\"type\":\"string\"}}}},"
