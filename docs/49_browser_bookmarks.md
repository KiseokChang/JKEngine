...
- **T2 NOTE 이월**: 폰트 미적재 단정 부재 — 형제 5앱 동일 관례라 단독 픽스
  ...
  **→ 2026-09-16 후속**: 최종리뷰 비차단 제안 반영 — anti-placeholder 게이트
  임계 80→20 완화(2px 스트라이드 샘플링+40/채널 기준에서 AA 래스터 변동이
  임계를 흔들 수 있음; 게이트의 역할은 AA 노이즈 대파만 하면 충분). 이후
  공식런은 16/16. 토글 스타 e2e의 간헐 실패는 두 클릭 사이 1s 타이밍 마진
  플레이크로 판정 — 토글-off 상태 스냅샷+list_windows 진단 추가로 정착 시간
  확보, 클린 런 2연속 0 failures).
