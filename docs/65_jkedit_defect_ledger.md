...
| O2 | 클라→서버 IME 강제 채널 부재 — 클라 모드 SilenceOsIme no-op, F2 진입 시 OS IME 영문 강제 무효(이중 조합 위험) | docs/61:334-337, 402-403 |
...
- **O2가 가장 큰 레저** — 내부 모드 진입 시 OS IME 강제 채널이 서버 창에 닿지
  않는 것은 이중 조합 위험을 남긴다(단일 소유 규칙 §18이 방어선).
