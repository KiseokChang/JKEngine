...
- **신규 최상위 ①: 재생 표시 페이싱 캡 규명** — 재생 중 표시 pop이
  콘텐츠-fps의 ~0.5×(4K50 26-31Hz, 480p30 15-16Hz)로 떨어지고 조그 경로는
  무영향. 후보(코드 근거, 미검증 가설):
  (a) 렌더 스레드가 payload-driven — `JKRenderThread.cpp:230` `Run()`의
  `PopWait(Channel::Render, payload, 5)` (5ms 폴),
  (b) vplayer 프레임 클록 = 16ms 레거시 타이머 —
  `ClientVPlayerApp.cpp` `OnInit()`의 `SetTimerInterval(16)`,
  (c) videoQ→UI 펌프 pop 레이트의 페이싱(조그는 표시를 링에서 직접 가져
  같은 캡을 안 지난다 — §8 구조와의 차이가 단서).
- **v2 d3d11va 제로카피(스펙 §7) 파킹 유지** — 같은 페이싱 캡 아래에서는
  업로드 비용을 없애도 표시율이 안 오른다(§9.3 대조군이 그 증거). 캡
  규명 후 재판단.
