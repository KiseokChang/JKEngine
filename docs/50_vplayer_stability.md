...
### 10.1 커밋 (실측 `git log --oneline 24bf943..HEAD` — 6커밋)
...
| 시나리오 | 실측 |
|---|---|
| S1 재생 중 진입 케이던스 | 143(4.767s)→125(4.167s)→108(3.600s)→98(3.267s) — 초당 ~30 미디어 프레임 = 콘텐츠 fps 역보행, UI 클록 00:04→00:03 동행. 토글오프 스냅 **98==98**(표시 프레임에 정확 착지, 보행 정지). 단 트랜스포트 라벨 역전 관찰 → §10.5 ② |
...
2. **S1 트랜스포트 라벨 역전** (s1-tint "Pause"=재생 표시 / s1-off
   "Play"=일시정지 표시 — 기대의 정확한 역) — `st.paused`는 UI SetPaused
   호출에서만 변하는데 두 시점 사이에 도달 가능한 경로가 코드에 없다.
   가설: 같은 ended 래치 × `jogWasPlaying_` — 2113행이
   `!st.paused && !st.ended`로 판정. **미해결, v2 결함 가능성으로
   최종리뷰 회부 확정.**
...
6. **관찰은 게이트가 아니다** — 3건(§10.5)은 프로브 구조+회귀 전부 녹색인
   ...
   사전존재 노출이고, 라벨 역전만 v2 가능성이다 — 관찰을 회귀로 승격시키지
   않되, v2 가능성 항목은 최종리뷰 회부를 남긴다(라벨 역전이 그 사례).
