...
- **레슨**: ①Toolhelp 스냅샷도 수기 선언 관례 동일(`PROCESSENTRY32W`는
  ...
  없는 환경(숨김 기동)에서는 특히.
- **후속 → 해소 (2026-09-24 실측)**: 서버 인수 시 브리지는 기존 서버
  접속을 잃지만 **재기동은 불요** — PumpLoop가 이미 재접속 루프를 갖고 있다
  (jkbridge main.cpp PumpLoop: `IsConnected()` false → `Connect()` 재시도 +
  `SubscribeEvents(true)` 재구독, 400ms 주기). 실측 2경로: ①서버
  kill→재기동 ②인수 경로(기존 서버 생존 상태에서 신규 서버 기동) — 브리지
  무수정, WS 도구 호출(ping) 즉시 성공. 갱신 14의 "재기동 필요"는 예방적
  기록이었던 것. 서버 장기 다운 중에도 펌프가 계속 재시도하므로 복구는
  서버 복귀 직후 자동이다.
