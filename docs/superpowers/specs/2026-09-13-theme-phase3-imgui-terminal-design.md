...
- **JK계열↔ImGui계열의 통합 질문 (사용자 제기 2026-09-13)** — 단계 3이
  팔레트 봉합(같은 토큰을 읽는 형제)까지 끝내면 남는 것은 세 갈래:
  ① JKControl로 수렴(ImGui 앱 재작성 — 즉시모드 장점 상실), ② JKWidget
  추상층 아래 두 백엔드(설계 비용 최대), ③ 역할 분리 유지 + 팔레트
  봉합. **판단 시점은 P4 앱 SDK 계약** — engine 밖 앱 계약을 정의하는
  순간 "앱이 어느 위젯 계열을 얻는가"가 계약의 일부가 된다. 지금은
  결정하지 않는다 (선택지를 열어두는 것이 이 단계의 산출).
