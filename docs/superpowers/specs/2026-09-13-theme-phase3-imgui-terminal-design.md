...
| 직감 | **실행 (게이트 실측)**: 매핑표 리뷰만으로 Table*/Tab* 기본값 잔존은 안 보인다 — 클래식 테이블 헤더 (48,48,51) on (240,240,240)을 픽셀 실측이 잡았고 94692c8로 즉시 확장 | task-5-report Step 6, task-5fix |
| 직감 | **실행**: TabActive는 1.90.9+에서 TabSelected와 동일 슬롯의 별칭 — 동일 슬롯 이중 배정은 나중 배정이 승리. 죽은 라인은 제거가 아니라 forward-compat 주석으로 유지 | task-5fix-report, imgui.h:1822 |
| 직감 | **실행**: 계획의 alphaScale ×1.2 상태 변주 스케치는 구현 불가 — 알파는 1.0 상한. 밝기 변주는 Lighten(RGB, 클램프) 헬퍼의 전유물 | task-2-report, plans §2 스케치 |
...
- **브라우저 북마크 기능 (사용자 요청, 2026-09-13)** — ClientBrowserApp 기능
  요청. P4 흡수/SDK 계약 논의와 별도 트랙으로 기록 (docs/47 §6).
