...
(가장 싼 경로), 업로드는 16.5MB로 절반. SDL2가 D3D11 백엔드에서 NV12 텍스처를
...
| 상수 | `kJogRingMaxSecs = 10.0` / `kJogRingMaxBytes = 1.5GB` 유지 | 바이트 캡은 절대 메모리 바운드로서 그대로 유효. 부수 효과: 4K에서 링 커버리지 ~0.9s→~1.8s (2×) |
...
- 링이 잡는 풀 슬롯 33MB→16.5MB — 같은 1.5GB 캡에서 4K 커버리지 2배.
