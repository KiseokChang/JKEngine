...
| 실행 | **CEF OSR 브리지의 규율은 락 금지다** — 통합 루프가 앱 스레드에서 펌프되는 한 콜백=렌더 동일 스레드, g_ 브리지 + 프레임당 멤버 복사로 충분. 락 도입은 단일 스레드 가정을 "미확정 멀티스레드 설계"로 바꾸고 InitCef/g_tex 등 기존 전역 전체를 재검토하게 만든다. 가정은 멤버 선언부 주석으로 문서화 | T1 구현 (02e1163), ledger 전역 제약 |
| 실행 | **.bak는 최신 백업이 아니라 파손 전 원본이다** — 매 저장마다 백업하면 파손 파일이 .bak를 덮어써 안전망이 소실. ledger 룰링 2: 최초 덮어쓰기 시점 1회 보존(이후 저장 무갱신), 파손 파일은 설계상 백업하지 않는다. 프로브가 존재 시점 + 이후 저장 불변까지 단정 | T2 구현 (593a1e3), 실측 e2e 12/12 |
| 실행 | **리뷰 구속 조건은 다음 태스크의 디스패치 요구로 이월된다** — T1 리뷰 3건(폰트/디포커스 배선/e2e 하네스)을 T2 brief 구속 조건으로 명시하자 폰트 미적재 MAJOR 승격 경로가 사전 봉쇄됐고 디포커스 퀴크는 IsItemFocused 게이트 픽스(593a1e3)로 귀결. 조건을 계약에 넣는 것이 리뷰 산출물의 소멸 방지 장치 | T1/T2 리뷰 (progress.md) |
| 실행 | **프로브 단정은 페이지를 구분해야 값어치가 있다** — T2 step-3는 타이핑 URL과 북마크 URL이 같은 페이지라 북마크 클릭 내비게이션을 단정 불가(회귀가 픽셀에 안 보임). 픽스: 서로 다른 URL 미리 타이핑 + 검출 신호를 페이지 픽셀이 아니라 별 활성 플립(stale 재내비게이션의 신호)+힌트 텍스트로 교체 (8d6f7bc, 16/16) | T2 리뷰 MINOR-1 → T3 게이트 흡수 |
| 실행 | **OnInit 적재는 lazy 초기화 지점에 걸리면 안 된다** — LoadBookmarks가 InitCef보다 먼저 돌므로 InitCef가 채우는 g_exeDirSlash 캐시 의존 시 빈 디렉터리를 읽는다. ExeDirSlash()는 캐시를 공유하되 수요 시 계산으로 분리. g_viewPageY 상시화와 같은 교훈의 다른 면: 1회성 초기화에 걸린 값은 전부 상시화 후보 | T2 구현 (593a1e3) |
