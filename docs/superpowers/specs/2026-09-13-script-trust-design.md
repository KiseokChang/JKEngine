...
- revoke는 파일 편집 + `triggers.reload` — 도구 제공 안 함(스코프 밖, 필요성 낮음:
  내용이 바뀌면 지문 자체가 달라져 자동 비신뢰).
...
- 신뢰 판정은 로드 시점마다 다시 한다 — trust.json 편집·reload가 즉시 반영된다.
...
- 로드 중 승인 대기는 블로킹 — 부팅 시 무해하나 `triggers.reload` 중간 로드 시
  펌프 최대 60초 정지 (saveLayout 블로킹 예례와 동일 등급).
