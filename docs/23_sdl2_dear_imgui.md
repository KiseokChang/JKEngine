...
- **A/V 오프셋 UI**: −1..+1s 슬라이더. 양수=오디오 늦게(비디오가
  ...
  `dropBeforePts`를 `t − 0.05 + min(0, avDelay)`로 확장하고, 착지
  키프레임이 그 게이트 창 안에 떨어지는 경우(GOP가 창보다 클 때
  흔함)를 `postSeekJump` 폴백이 막는다 — 시크 직후 첫 팝 시도에서
  큐 head가 게이트보다 미래면 착지 프레임을 1회 강제 표시(사용자
  도깨비 E01 −0.71s 시크 프리즈 보고로 수정; 1차만으로는
  키프레임 착지가 운에 좌우되어 반은 여전히 동결). 양수는 게이트가
  미래라 불필요.
...
판정. 음수 A/V(−0.78) + 일시정지 시크는 ffmpeg 추출 참조 프레임과
번인 카운터/색상 위상을 대조해 확인(착지 키프레임 즉시 표시, 재생
재개·av=0 회귀 포함).

**스모크 도구 교훈(추가):** `cmake --build build --target
jkapp_vplayer`는 DLL만 빌드한다 — 클라이언트가 실행하는 것은
`build/apps/vplayer.jkx`의 MODL이므로, 재빌드 후에는 반드시
`--target jkx_packages`(또는 디폴트 ALL)로 재팩까지 해야 스폰이
새 코드를 추출한다. 재팩 없이 재스폰하면 "픽스가 안 됐다"는
오판을 하게 된다(실제로 1회 발생).

조그 프레임 스크럽 (2026-09-15, docs/50 §8) — 무음 프레임 단위
앞/뒤 조그 + 10초 히스토리 링 + 링 밖 키프레임 폴백.
