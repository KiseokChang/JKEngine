...
의도적 잔존 2건 (토큰화 클러스터 내) + 토큰화 밖 유사 사이트 1건:

1. **테트리스 퍼플 (128,0,128)** — 앱 식별색은 테마 토큰이 아니라 개성.
2. **클라이언트 배경 (240,240,240)** — 위젯 표면으로 단계 2 영역. 지금
   바꾸면 Win95 실버 위젯 위에 다크 배경만 얹는 혼재가 심해진다.
3. **(토큰화 밖) JKClientApplication.cpp:491 클라 표면 클리어 (192,192,192)**
   — 문서화된 2건과 같은 "위젯 표면" 카테고리지만 단계 1 대상
   클러스터 밖이라 미스왑. **단계 2 체크리스트에 선반영** — 나중에
   "토큰 누수"로 재발견되는 것을 방지.

예약 토큰 2종 (현재 미사용, 값만 확정):

- **chromeActiveBorder (0,120,212) `#0078D4`** — 소비처(활성 창 경계)는
  후속 과제.
- **chromeCloseHover (196,43,28)** — 닫기 버튼 **호버 상태 자체가
  아직 미구현**(호버 경로는 커서 변경만 존재)이라 소비처가 없다.
  호버 페인트를 단계 2/3에서 구현할 때 소비.

태스크바 칩 팔레트 비고: `TitleChipColor`의 FNV-1a 해시색(96..159/채널)
은 스펙 §2-5대로 절차색 유지 — 개인화 목적이라 "kDefault 범위로의
팔레트 정렬"은 실시하지 않았다 (스펙 문구의 남은 모호함 해소).
...
- JKTheme.h 집합 초기화의 위치 의존 위험 — 20필드 주석 라벨이 유일한
  방어. 중간 필드 삽입 시 이후 값 전부 침묵히 이동. C++20 지정 초기자
  전환 또는 최소한 단계 2 기여자 가시 위치의 경고로 대응

다음: **단계 2 위젯 라이브러리** (JKDC 패스스루 — `JKDC::Box3D/
Rectangle3D`는 색을 인자로 받아 통과 지점이 이미 존재; 위 사이트 목록
3번의 JKClientApplication 표면 클리어 포함). 이후 **단계 3**에서 ImGui
앱 스타일(PushStyleColor 래퍼)과 터미널 통합 재판단.
