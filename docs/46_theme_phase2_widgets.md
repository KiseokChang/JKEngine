...
3. **닫기/최대화 버튼 윤곽 — 구값 (0,0,0) → kClassic에서 192-on-192로
   소멸(보이지 않음)**. 단계 1에서 버튼 윤곽(0,0,0)과 창 테두리(192)가
   `chromeBorder` 단일 토큰으로 통합된 결과(docs/45 §3) — kClassic은 창
   테두리 쪽 진실값(192)을 복원하므로 버튼 윤곽만 암시(implicit) 실패.
   코드 변경 불요(토큰 통합의 본질) — 회귀 오라클로서의 기록 목적.
   게이트 미측정 표면.
...
- **JKTheme.h 집합 초기화의 위치 의존 위험 (계승 + 증폭)** — 단계 1의 열린
  마이너(docs/45 §6)를 계승하되 구조체가 20→35필드로 커져 위험도 증폭.
  현재는 3프리셋 × 35필드 주석 라벨이 유일한 방어(최종 리뷰 기계 검증
  35/35/35 정합). C++20 지정 초기자 전환이 근본 해소 — 단계 3 기여자
  첫 손길로 권장. 로더의 첫 `"preset"` 문자열 매칭 시맨틱도 키 확장 시
  재검토 대상.
